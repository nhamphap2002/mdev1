...
class Mysql {
    ...
    function __construct($option) {
        ...
        if (function_exists('mysql_connect')) {
            if (!($_resource = mysql_connect($dbHost, $dbUser, $dbPassword, true))) {
            ...
            if (!($_resource = mysqli_connect($dbHost, $dbUser, $dbPassword, $dbDatabaseName))) {
                //Log::error("Could not connect to MySQL.");
                echo "Could not connect to MySQL.";
                return;
            }
        ...
        if (!mysqli_select_db($_resource, $dbDatabaseName)) {
    ...
    public function query($query) {
        mysqli_query($this->_resource, TIMEZONE);
        $result = mysqli_query($this->_resource, $query);
    ...
    public function querySql($query, $dcall = '') {
        ...
        if ($result == false) {
            ...
            $this->_errorNum = mysqli_errno($this->_resource);
            //Log::error(mysqli_errno($this->_resource) . ": " . mysqli_error($this->_resource) );
            //Log::error($query);
        }
    ...
    public function insertSql(array $fields_values, $table) {
        ...
        //echo sprintf( $fmtsql, implode( ",", $fields ) ,  implode( ",", $values ) );exit();
...
//end function deleteSql
    ...
    public function fetchArray($cursor = null) {
        return mysqli_fetch_row($cursor ? $cursor : $this->_cursor);
    }

    public function fetchObject($cursor = null, $class = 'stdClass') {
        return mysqli_fetch_object($cursor ? $cursor : $this->_cursor, $class);
    }

    public function freeResult($cursor = null) {
        mysqli_free_result($cursor ? $cursor : $this->_cursor);
    ...
    public function escapeString($str) {
        return mysqli_real_escape_string($this->_resource, $str);
    }
